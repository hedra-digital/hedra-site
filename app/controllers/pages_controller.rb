...
class PagesController < ApplicationController
  ...
  def home
    ...
    @features        = Feature.includes(:page).joins(:book).joins(:features_publishers).includes(:book => { :participations => [:person, :role] }).where("features_publishers.publisher_id = #{session[:publisher]}").first(6)
